...
5. `src/main/agent/tools/ToolExecutor.ts`
   ...
   - confirmation 执行前读取 active run mutable runtime context 中的 `permissionApprovalMode`，运行中从 manual 切到 auto 会影响后续 tool call。
   - 已进入 pending 的 confirmation 在切到 auto 后由 run runtime 主动 approve，避免等待 5 分钟 timeout。
