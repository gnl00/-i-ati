...
<userData>/logs/scheduler-YYYY-MM-DD.log
...
~/Library/Application Support/at-i-app/logs/scheduler-2026-03-20.log
...
- `scheduler-YYYY-MM-DD.log`
  - 定时任务调度控制面日志
  - 记录 `SchedulerService` 的 tick、任务领取、执行结果和调度错误
  - 固定保留 debug 级别，支持观察空 tick 和下一任务计时器
...
- `scheduler-YYYY-MM-DD.log` 和普通日志一样按天切分，并参与压缩与清理
...
- 启动性能链通过 `createPerfLogger(scope)` 单独写入 `perf-YYYY-MM-DD.log`
- 定时任务调度控制面通过 `createSchedulerLogger(scope)` 单独写入 `scheduler-YYYY-MM-DD.log`
...
} from '@main/logging/LogService'

const logger = createLogger('MyService')
const perfLogger = createPerfLogger('Startup')
const schedulerLogger = createSchedulerLogger('SchedulerService')
...
schedulerLogger.debug('tick.claimed_due_tasks', { count: 0 })
```

每个 scoped logger 独占对应 target。`SchedulerService` 的生命周期事件进入 scheduler 文件；定时任务触发的聊天运行、模型请求和工具调用继续由各自 scope 写入 app/request 文件。
...
- 截断
- 错误序列化
- app、perf、scheduler target 路由
- 压缩清理
- app、perf、request、scheduler 文件查询
