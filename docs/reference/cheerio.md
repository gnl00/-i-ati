...
cheerio实现了核心jQuery的子集。cheerio会从jQuery库中删除所有DOM矛盾以及浏览器的尴尬部分,展现它真正完美的API。

<!-- __&#991; Blazingly fast:__ -->
__&#991; 非常快:__
<!-- Cheerio works with a very simple, consistent DOM model. As a result parsing, manipulating, and rendering are incredibly efficient. Preliminary end-to-end benchmarks suggest that cheerio is about __8x__ faster than JSDOM. -->
cheerio使用一个非常简单且一致的DOM模型。将其作为一个用于结果分析、处理以及呈现的工具非常有效。基础的端到端基准测试显示其速度为JSDOM的 __8x__ 倍。
...
cheerio基于 @FB55's forgiving [htmlparser2](https://github.com/fb55/htmlparser2/)。这使得cheerio可以解析几乎任何HTML或XML文档。

<!-- ## What about JSDOM? -->
## 关于JSDOM?
<!-- I wrote cheerio because I found myself increasingly frustrated with JSDOM. For me, there were three main sticking points that I kept running into again and again: -->
我之所以开发了cheerio主要是因为我发现我自己对JSDOM非常失望。于我而言,(JSDOM)主要有三个点令我一直耿耿于怀:

<!-- __&#8226; JSDOM's built-in parser is too strict:__ -->
__&#8226; JSDOM内置的解析器过于严格:__
  
  JSDOM内置的HTML解析器无法处理现今许多流行的网站。

<!-- __&#8226; JSDOM is too slow:__ -->
__&#8226; JSDOM 太慢了:__
<!-- Parsing big websites with JSDOM has a noticeable delay. -->
使用JSDOM解析大网站时有着明显的延迟。
...
JSDOM的设计旨在提供一个正如我们在浏览器中所看到的相同DOM环境。我从未真正需要过所有这些,我只是想要一个简单且熟悉的HTML操作方法。
...
cheerio无法解决你所有的问题。我仍然会使用JSDOM如果我需要工作在服务器(后端)的浏览器环境中,特别是如果我想进行自动化功能测试。
## API
...
首先你需要加载HTML。在jQuery中这一步是必须的,因为一个jQuery要想运行,就常常需要用到DOM。在cheerio中,我们需要把HTML document传进去。
...
如果你需要修改默认的解析设置你也可以传递一个额外的对象给`.load()`:
...
这些解析选项直接取自[htmlparser2](https://github.com/fb55/htmlparser2/wiki/Parser-options),因此你也可以在cheerio中使用任何在`htmlparser2`中有用的选项(设置)。默认的选项(设置)是:
...
至于一个完整的选项列表和他们的影响,请看 [这里](https://github.com/fb55/DomHandler)和
[htmlparser2's options](https://github.com/fb55/htmlparser2/wiki/Parser-options).
...
cheerio和jQuery选择器的实现几乎是相同的,因此API非常相似。

#### $( selector, [context], [root] )
<!-- `selector` searches within the `context` scope which searches within the `root` scope. `selector` and `context` can be a string expression, DOM Element, array of DOM elements, or cheerio object. `root` is typically the HTML document string. -->

<!-- This selector method is the starting point for traversing and manipulating the document. Like jQuery, it's the primary method for selecting elements in the document, but unlike jQuery it's built on top of the CSSSelect library, which implements most of the Sizzle selectors. -->
`selector` 在 `root` 的范围内搜索 `context` 。`selector`和`context`可以是一个字符串表达式,DOM元素,DOM元素的数组,或cheerio对象。`root` 通常为HTML文档字符串。

开发这个选择器方法的出发点是用来遍历和操作文档。就像jQuery,它是选择文档中元素的主要方法,但和jQuery不同的是，它是基于CSSSelect库来实现大部分的选择器的。
...
用于获取和设置属性的方法。在匹配集合中只能获取的第一个元素的属性值。如果你把属性值设置为空，就会删除该属性。你也可以像jQuery一样通过传递`键值` 和 `函数`来设置。
...
> 详见 http://api.jquery.com/attr/

#### .prop( name, value )
<!-- Method for getting and setting properties. Gets the property value for only the first element in the matched set. -->
用于获取和设置属性的方法。用于获取唯一匹配集的第一元素的属性值。
