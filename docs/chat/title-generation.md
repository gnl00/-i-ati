...
> **状态**: 运行中参考档（持续同步） · **最后更新**: 2026-07-02

本文记录 Title Agent 当前实现事实：`TitleJobService.run()` 使用 `agent()` 抽象路径，标题任务默认启用，模型选择走 Provider 三槽配置中的 `liteModel`。

---

## 1. 当前实现

`TitleJobService.run()` 现在以以下方式执行：

1. 由 `shouldRun()` 判断会话标题是否仍为默认标题。
2. 通过 `resolveLiteModelRef(config)` 解析轻量任务模型上下文，缺省链路回退到 `mainModel` 与可用模型。
3. 配置上下文解析失败时沿用本次主 run 的模型上下文。
4. 调用
...
- 标题模型与 Smart Message、Scheduler 等后台轻量任务共享 `liteModel` 槽位；Telegram 聊天消息走 `mainModel` 聊天链路。
...
- 当前 `TitleJobService` 走 `agent()` 单一路径。
- `shouldRun` 的 title 再次触发条件依赖会话标题是否为 `NewChat`。
...
- [x] `src/shared/services/ChatModelResolver.ts`：`resolveLiteModelRef` 统一轻量任务模型选择。

---

## 7. 回归覆盖

- `src/main/orchestration/chat/postRun/__tests__/TitleJobService.test.ts` 覆盖默认标题、已有标题、失败、text 响应、最近上下文拍扁、`liteModel` 上下文选择。
