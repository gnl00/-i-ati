...
- 旧 loop 已移除；对应复杂度现在应只在 `runtime` / host adapter 边界里被显式建模
...
> 进展（2026-07，render pipeline P0）：host 侧「影子 reducer」重复已消除。
> 此前 `HostRenderEventMapper` 内的 `AgentRenderStateReducer` 与
> `ChatRenderResponder` / `TelegramRenderResponder` 各自持有的 `HostRenderStateController`
> 存的是同构的 committed/preview 状态（同一份被建模两次）。P0 后 `HostRenderStateController`
> 已删除，host 侧 render 状态收敛到 `HostRenderEventMapper` 这唯一一个 fold 点。
...
## 9. 当前 runtime 需要优先解决什么

当前 runtime 优先解决的不是“换一批文件名”，而是下面几个根因：
...
> 修正（2026-07，render pipeline P0/P1）：关于「output 层归属」的结论已按依赖边界修正。
> `host/output/` 位于 core runtime 内，而真实 output 产出的是 host-visible 契约
> （`@shared/chat/render-events` 的 segment / patch），若在此实现会把 chat 专属渲染契约
> 拉进 core runtime。因此结论修正为：
>
> - output 层合法归属在 `hosts/shared/render` + `hosts/*/runtime`；
