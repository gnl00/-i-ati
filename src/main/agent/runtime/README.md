...
- `AgentRuntime` 不拥有独立于 `AgentLoop` 之外的业务状态机
...
`AgentRuntime.run(...)` 的内部 wiring 顺序应固定为：

1. 接收 `AgentRuntimeRunInput`
...
- `AgentRuntime` 自己不持有第二套 run state
...
- `AgentRequestSpec` 应基于显式的 `AgentRuntimeRunInput` 做 per-run resolve，而不是依赖隐式环境状态
- `LoopRunDescriptor` 应基于显式的 `AgentRuntimeRunInput` 做 per-run create，再传给 bootstrap 和 loop
- `AgentRuntimeContext` 当前保持平铺结构，但阅读上应按下面几组理解：
  - `sources`
    - `requestSpecSource`
    - `runDescriptorSource`
  - `bootstrap`
