...
export async function initializeBuiltInAssistants(): Promise<void> {
  try {
    ...
    if (hasBuiltInAssistants) {
      console.log('[AssistantBootstrap] Built-in assistants already exist, skipping initialization')
      return
    }

    // 创建所有内置助手
    console.log(`[AssistantBootstrap] Creating ${BUILT_IN_ASSISTANTS.length} built-in assistants`)
    const now = Date.now()
    ...
    console.log('[AssistantBootstrap] Successfully initialized all built-in assistants')
  } catch (error) {
    console.error('[AssistantBootstrap] Failed to initialize built-in assistants:', error)
    throw error
  }
}
