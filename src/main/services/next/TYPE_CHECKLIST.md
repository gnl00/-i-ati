...
这份文档把 `SCENARIOS.md` 里的场景，映射到 `next` 当前已经定义的 contract。
...
- `runtime/NextAgentRuntimeRunInput`
- `runtime/NextAgentRuntimeContext`
...
`NextAgentRuntimeContext` 至少应显式持有：
...
`NextAgentRuntime` 至少应显式暴露：

- `run(input)`
  - `NextAgentRuntimeRunInput`
  - 内部顺序应固定为：
