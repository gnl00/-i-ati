...
import { TaskPlanRepository, TaskPlanRow, TaskPlanStepRow } from '../db/repositories/TaskPlanRepository'
...
class DatabaseService {
  ...
  public saveTaskPlan(plan: Plan): void {
    if (!this.db) throw new Error('Database not initialized')
    if (!this.taskPlanRepo) throw new Error('Task plan repository not initialized')
    const now = Date.now()
    ...
    this.taskPlanRepo.insertPlan(row)
  ...
  public updateTaskPlan(plan: Plan): void {
    ...
    this.taskPlanRepo.updatePlan(row)
  }

  public updateTaskPlanStatus(id: string, status: PlanStatus, currentStepId?: string, failureReason?: string): void {
    if (!this.db) throw new Error('Database not initialized')
    if (!this.taskPlanRepo) throw new Error('Task plan repository not initialized')
    this.taskPlanRepo.updatePlanStatus(id, status, currentStepId ?? null, failureReason ?? null, Date.now())
  }

  public getTaskPlanById(id: string): Plan | undefined {
    if (!this.db) throw new Error('Database not initialized')
    if (!this.taskPlanRepo) throw new Error('Task plan repository not initialized')
    const row = this.taskPlanRepo.getPlanById(id)
    if (!row) return undefined
  ...
  public getTaskPlansByChatUuid(chatUuid: string): Plan[] {
    if (!this.db) throw new Error('Database not initialized')
    if (!this.taskPlanRepo) throw new Error('Task plan repository not initialized')
    const rows = this.taskPlanRepo.getPlansByChatUuid(chatUuid)
  ...
  public deleteTaskPlan(id: string): void {
    if (!this.db) throw new Error('Database not initialized')
    if (!this.taskPlanRepo) throw new Error('Task plan repository not initialized')
    this.taskPlanRepo.deletePlan(id)
  }
  ...
  public saveTaskPlanSteps(planId: string, steps: PlanStep[], createdAt?: number, updatedAt?: number): void {
    ...
    for (const step of steps) {
      ...
      this.taskPlanRepo.insertStep(row)
    }
  }
  ...
  public upsertTaskPlanStep(planId: string, step: PlanStep): void {
    ...
    this.taskPlanRepo.upsertStep(row)
  }
  ...
  public updateTaskPlanStepStatus(
    ...
    if (!this.db) throw new Error('Database not initialized')
    if (!this.taskPlanRepo) throw new Error('Task plan repository not initialized')
    this.taskPlanRepo.updateStepStatus(
      planId,
      stepId,
      status,
  ...
  private getTaskPlanSteps(planId: string): TaskPlanStepRow[] {
    if (!this.taskPlanRepo) return []
    return this.taskPlanRepo.getStepsByPlanId(planId)
  }
