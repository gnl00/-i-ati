...
import { assertMessageEntitySegmentsHaveIds } from '@shared/chatRun/segmentId'
...
export class AgentUiAdapter implements AgentEventSink {
  ...
  constructor(
    private readonly emitter: import('@main/services/chatRun/infrastructure').ChatRunEventEmitter,
  ...
  async handle(event: AgentEvent): Promise<void> {
    ...
    switch (event.type) {
      case 'step.started':
        this.emitter.emit(CHAT_RUN_EVENTS.RUN_STATE_CHANGED, { state: 'streaming' })
        return
      case 'step.delta':
        if (event.delta.type === 'tool_call_ready') {
          this.emitter.emit(CHAT_RUN_EVENTS.TOOL_CALL_DETECTED, {
      ...
      case 'tool.awaiting_confirmation':
        this.emitter.emit(CHAT_RUN_EVENTS.TOOL_EXEC_REQUIRES_CONFIRMATION, {
  ...
  private async handleToolProgress(
    ...
    if (event.phase === 'started') {
      ...
      this.emitter.emit(CHAT_RUN_EVENTS.RUN_STATE_CHANGED, { state: 'executing_tools' })
