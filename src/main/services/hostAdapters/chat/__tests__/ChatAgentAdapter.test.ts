...
describe('ChatAgentAdapter', () => {
  ...
  it('emits chat preparation events during prepareRun', async () => {
    ...
    expect(emitter.emit).toHaveBeenCalledWith(CHAT_RUN_EVENTS.CHAT_READY, {
      chatEntity: prepared.chatContext.chat,
      workspacePath: prepared.chatContext.workspacePath
    })
    expect(emitter.emit).toHaveBeenCalledWith(CHAT_RUN_EVENTS.MESSAGES_LOADED, {
      messages: prepared.chatContext.historyMessages
    })
    expect(emitter.emit).toHaveBeenCalledWith(CHAT_RUN_EVENTS.MESSAGE_CREATED, {
