...
function resolveWorkspaceContainedInput(
  ...
  const lexicalPath = isAbsolute(inputPath)
    ? resolve(inputPath)
...
export function resolveWorkspacePath(
  ...
  try {
    ...
    const lexicalInside = isPathWithin(lexicalPath, lexicalWorkspaceRoot)
    const canonicalRootAliasInside = isPathWithin(lexicalPath, canonicalWorkspaceRoot)
    const boundaryInside = lexicalInside || canonicalRootAliasInside
    if (options.mode === 'workspace-contained' && !boundaryInside) {
      throw new WorkspacePathError(
        'PATH_OUTSIDE_WORKSPACE',
