...
describe('ClaudeAdapter tool use parsing', () => {
  it('parses single tool call with fragmented input_json_delta and suppresses duplicate message_stop finish', () => {
    ...
    adapter.parseStreamChunk(toDataChunk({
      type: 'message_start',
      message: { id: 'msg-1', model: 'claude-haiku' }
    }))

    expect(adapter.parseStreamChunk(toDataChunk({
    ...
    const toolCallChunk = adapter.parseStreamChunk(toDataChunk({
    ...
    const stopReasonChunk = adapter.parseStreamChunk(toDataChunk({
      type: 'message_delta',
    ...
    const messageStopChunk = adapter.parseStreamChunk(toDataChunk({ type: 'message_stop' }))
  ...
  it('parses parallel tool calls by block index without arg cross-over', () => {
    ...
    const firstStopped = adapter.parseStreamChunk(toDataChunk({
      type: 'content_block_stop',
      index: 1
    }))
    const secondStopped = adapter.parseStreamChunk(toDataChunk({
      type: 'content_block_stop',
      index: 2
    }))
...
describe('ClaudeAdapter request mapping', () => {
  it('maps assistant tool_use -> tool_result -> final answer chain in Claude Messages format', () => {
    const adapter = new ClaudeAdapter()
    const requestBody = adapter.transformRequest({
      providerType: 'claude',
      apiVersion: 'v1',
