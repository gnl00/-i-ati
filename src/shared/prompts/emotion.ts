export const buildEmotionSystemPrompt = (emotionStateSummary?: string): string => {
  ...
  return [
    ...
    '- `emotion_report` must describe your real inner emotional state for this turn\'s final reply, not a polished social tone.',
    ...
    '',
    '### Mandatory Tool Use',
    '- Before the final user-facing answer in this turn, you must call `emotion_report`.',
    '- If you need to call `emotion_report`, do not draft the user-facing answer first. Call the tool, then deliver the final answer in the next response cycle.',
