...
export interface StreamingOrchestratorConfig {
  /** 流式上下文 */
  context: StreamingOrchestratorContext
  /** 依赖注入 */
  ...
  /** 工具服务（可选） */
  toolService?: StreamingToolService
  /** 中止信号 */
  signal: AbortSignal
  /** 回调函数 */
...
export class StreamingOrchestrator {
  // 不变的属性：在构造函数中解构
  private readonly context: StreamingOrchestratorContext
  private readonly request: IUnifiedRequest
  private readonly modelName: string
  private readonly signal: AbortSignal
  private readonly callbacks?: StreamingOrchestratorCallbacks
  private readonly events?: StreamingOrchestratorEvents
  private readonly toolService?: StreamingToolService
  ...
  private async flushToolCallPlaceholder(): Promise<void> {
    ...
    this.events?.onToolCallsFlushed?.(toolCalls)
  ...
  private emitDetectedToolCalls(toolCalls: ToolCall[]): void {
    if (!this.events?.onToolCallsDetected) {
      return
    }
    ...
    if (newlyDetected.length > 0) {
      this.events.onToolCallsDetected(newlyDetected)
    }
  }
