...
import { PrepareMessageFn, PrepareMessageParams, PreparedChat } from './types'
...
/**
 * Prepare the message and workspace
 * @param input
 * @param model
 * @param chat
 * @param store
 ...
 * @returns
 */
export const prepareMessageAndChat: PrepareMessageFn = async ({
  input,
  model,
  chat,
  store,
  ...
  const { textCtx, mediaCtx, tools } = input
  const {
    chatId,
    chatUuid,
    setChatId,
