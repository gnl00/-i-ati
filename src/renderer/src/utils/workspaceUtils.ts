 ...
 * @returns 工作空间的相对路径
 */
export function getWorkspacePath(chatUuid?: string): string {
  const workspaceName = chatUuid || WORKSPACE_CONFIG.defaultWorkspace
  const workspacePath = `${WORKSPACE_CONFIG.baseDir}/${workspaceName}`
  console.log(`[Workspace] Current workspace path: ${workspacePath} (chatUuid: ${chatUuid || 'default'})`)
 ...
 */
export async function createWorkspace(chatUuid?: string): Promise<{
  success: boolean
  path: string
  error?: string
}> {
 ...
 */
export async function switchWorkspace(chatUuid?: string): Promise<{
  success: boolean
  path: string
  created: boolean
  error?: string
  ...
  try {
    ...
    const createResult = await createWorkspace(chatUuid)
