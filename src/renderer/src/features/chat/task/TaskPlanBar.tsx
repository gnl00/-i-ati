...
import { taskPlannerService } from '@renderer/features/task-planner'
...
export function TaskPlanBar({
  ...
  const { completedCount, activeStep, failedStep } = useMemo(() => {
    return {
      completedCount: plan.steps.filter(step => step.status === 'done').length,
    ...
    >
      {isPendingReview ? (
            ...
            <div className="flex min-w-0 flex-1 items-center gap-3">
              ...
              <div className="min-w-0 flex-1 space-y-1">
                ...
                <div className="flex flex-wrap items-center gap-x-3 gap-y-1 text-[10.5px] text-slate-600 dark:text-slate-300">
                  <span>{completedCount}/{plan.steps.length} complete</span>
                  <span>{plan.steps.length} planned</span>
                </div>
              </div>
            </div>
              ...
              <div className="flex items-center gap-1 rounded-full border border-white/30 bg-gray-100 p-1 shadow-[0_10px_30px_-20px_rgba(15,23,42,0.45)] backdrop-blur-md dark:border-white/10 dark:bg-slate-900/55">
                <Button
                  ...
                  className={cn(
                    ...
                    'dark:text-rose-300 dark:hover:bg-rose-950/40 dark:hover:text-rose-200',
                    'disabled:opacity-50'
                  )}
                >
                  Reject
        ...
        <div className="flex items-center gap-3 px-4 py-2">
          ...
          <span className="shrink-0 text-[11px] text-slate-600 dark:text-slate-300">
