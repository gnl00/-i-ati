...
import { useScrollManagerV3 } from '@renderer/hooks/useScrollManagerV3'
...
const ChatWindowComponentNext3: React.FC = () => {
  ...
  const {
    ...
    onRangeChanged,
    triggerJumpToLatest
  } = useScrollManagerV3({ messages, chatUuid, readStreamState })
  ...
  const handleJumpToLatestClick = useCallback(() => {
    ...
    const isLatestAssistant = lastAssistantIndex === messages.length - 1
    ...
    triggerJumpToLatest()
  }, [lastAssistantIndex, messages, readStreamState, triggerJumpToLatest, updateMessage, upsertMessage])
  ...
  return (
    <div className="min-h-svh max-h-svh overflow-hidden flex flex-col app-undragable bg-chat-light dark:bg-chat-dark">
      ...
      <ResizablePanelGroup
        ...
        id="vertical-panel-group-next3"
      >
        <ResizablePanel
          id="main-content-panel-next3"
          defaultSize={75}
          minSize={30}
          maxSize={85}
        ...
        >
          <ResizablePanelGroup
            ...
            id="horizontal-panel-group-next3"
          >
            <ResizablePanel
            ...
            {artifactsPanelOpen && (
              <>
                <ResizableHandle className="hover:bg-primary/10 active:bg-primary/20 bg-transparent transition-colors duration-200 mt-2 mb-2 [&>div]:hidden [&::before]:hidden" />
                <ResizablePanel
                  defaultSize={40}
                  minSize={25}
                  maxSize={70}
...
export default ChatWindowComponentNext3
