...
import { Check, Code, ExternalLink, Server, Trash2 } from 'lucide-react'
          ...
          <div className="p-4 flex items-start justify-between gap-4 flex-1">
            <div className="flex-1 min-w-0 relative">
              <div className="flex items-start gap-4 mb-3">
                <div className="h-11 w-11 rounded-xl bg-gray-50 dark:bg-gray-800/50 flex items-center justify-center flex-shrink-0 border border-gray-100 dark:border-gray-700/50 transition-all duration-300 shadow-sm group-hover:scale-105 will-change-transform">
                  {iconUrl ? (
                    <img src={iconUrl} alt="" className="h-6 w-6 rounded-md opacity-90 group-hover:opacity-100 transition-opacity" onError={(e) => { e.currentTarget.style.display = 'none' }} />
                  ) : (
                ...
                <div className="flex-1 min-w-0 pt-0.5">
                  <div className="flex items-center gap-2 mb-1 flex-wrap">
                    <h4 className="font-bold text-[14px] leading-tight tracking-tight truncate text-gray-900 dark:text-gray-100">
                      {title?.substring(title.indexOf("/") + 1) || name.substring(name.indexOf("/" + 1))}
                    </h4>
            ...
            <div className="pt-0 flex-shrink-0 z-10">
              {installed ? (
                <Badge
                  variant="outline"
                  className="text-xs text-gray-600 dark:text-gray-400 select-none font-semibold px-3 py-1 uppercase tracking-tight rounded-md border-transparent bg-gray-100 dark:bg-gray-900/30 transition-all duration-300 flex items-center gap-1.5"
                >
                  Installed
                </Badge>
              ) : (
                <Button
                  ...
                  variant="ghost"
                  onClick={onInstall}
                  className="px-3 py-0.5 rounded-md text-xs font-semibold uppercase tracking-tight text-blue-600 hover:text-blue-800 dark:text-blue-300 bg-blue-50/80 dark:bg-blue-900/20 hover:bg-blue-100 dark:hover:bg-blue-900/40 transition-all duration-200 flex items-center gap-1.5"
                >
                  Install
                </Button>
              )}
