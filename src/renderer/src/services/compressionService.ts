...
} from '@renderer/db/CompressedSummaryRepository'
...
class MessageCompressionService {
  ...
  shouldCompress(messageCount: number): boolean {
    const config = useAppConfigStore.getState().compression
...
5. 重点关注用户的需求和助手的回答要点`
  ...
  async compress(
    ...
    try {
      ...
      // 3. 获取已有的压缩摘要
      const existingSummaries = await getActiveCompressedSummariesByChatId(chatId)
