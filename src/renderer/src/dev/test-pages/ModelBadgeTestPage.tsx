import { ModelBadgeV2 } from '@renderer/dev/experiments/model-badge/ModelBadgeV2'
import { ModelBadgeV3 } from '@renderer/dev/experiments/model-badge/ModelBadgeV3'
import { ModelBadge } from '@renderer/features/chat/message/assistant-message/model-badge/ModelBadge'
import { ReasoningSegment } from '@renderer/features/chat/message/assistant-message/segments/ReasoningSegment'
...
const mockToolCallSegment: ToolCallSegment = {
  ...
  content: {
    ...
    status: 'completed',
    args: {
      file_path: 'src/renderer/src/dev/experiments/model-badge/ModelBadgeV2.tsx',
      start_line: 1,
      max_lines: 80
    },
    result: {
    ...
    <div className="relative min-h-screen overflow-hidden bg-slate-100 text-slate-900 dark:bg-slate-950 dark:text-slate-100">
      ...
      <div className="relative mx-auto flex min-h-screen w-full max-w-6xl flex-col gap-8 px-6 py-10">
        ...
        <div className="grid gap-6 xl:grid-cols-2">
          ...
          <DemoAssistantBubble
            title="Fresh Next"
            badge={<ModelBadge model="MiniMax-M2.5" provider="minimax" animate />}
          />
        </div>
        ...
        <div className="grid gap-6 xl:grid-cols-2">
          <DemoAssistantStack
            title="Baseline Stack With ModelBadge"
            badge={<ModelBadge model="MiniMax-M2.5" provider="minimax" animate />}
            reasoning={(
              <ReasoningSegment
                segment={mockReasoningSegment}
          ...
          />
          <DemoAssistantStack
            title="Matched Stack With ModelBadge"
            badge={<ModelBadge model="MiniMax-M2.5" provider="minimax" animate />}
            reasoning={(
              <ReasoningSegment
                segment={mockReasoningSegment}
